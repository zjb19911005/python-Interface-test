...
class testFSR_here(unittest.TestCase):
	...
	def test002_modify_cash(self):
		...
		cur = connect.cursor()
		# 获取数据库有多少条数据
		sut = cur.execute("select 1000*UNIX_TIMESTAMP(server_update_time) from trade where uuid = '%s'")% ("7bf82d54146450b98070d027f3f"+self.y),
		sct = cur.execute("select 1000*UNIX_TIMESTAMP(server_create_time) from trade where uuid = '%s'") % ("7bf82d54146450b98070d027f3f"+self.y),
